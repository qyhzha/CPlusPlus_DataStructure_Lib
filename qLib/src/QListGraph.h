#ifndef __QLISTGRAPH_H__
#define __QLISTGRAPH_H__
    ...
    public:
        ...
        int addVertex()
        {
            ...
            else
            {
                THROW_EXCEPTION(QNoEnoughMemoryException,
                                "No enough memory to create new Vertex QObject...");
            }
        ...
        bool setVertex(int i, const V &obj)
        {
            ...
            if (ret)
            {
                ...
                {
                    THROW_EXCEPTION(QNoEnoughMemoryException,
                                    "No enough memory to create new Vertex value...");
                }
            }
        ...
        {
            return ((0 <= i)
                    && (i < vCount())) ? (m_list[i].edge.insert(m_list[i].edge.length(), Edge<E>(i,
                                          j)), m_list[i].edge.length() - 1) : -1;
        }

        int addEdge(int i, int j, const E &obj)
        {
            return ((0 <= i)
                    && (i < vCount())) ? (m_list[i]->edge.insert(m_list[i]->edge.length(),
                                          Edge<E>(i, j, obj)), (m_list[i]->edge.length() - 1)) : -1;
        }
        ...
        {
            return ((0 <= i) && (i < vCount()) && (0 <= j)
                    && (j < m_list.get(i)->edge.length())) ? (m_list.get(i)->edge[j].data = obj,
                            true) : false;
        }
        ...
        {
            return ((0 <= i) && (i < vCount()) && (0 <= j)
                    && (j < m_list.get(i)->edge.length())) ? m_list.get(i)->edge.remove(j) : false;
        }
        ...
        int ID(int i) const
        {
            ...
            if ((0 <= i) && (i < vCount()))
            {
                for (m_list.move(0); !m_list.end(); m_list.next())
                {
                    for (m_list.current()->edge.move(0); !m_list.current()->edge.end();
                         m_list.current()->edge.next())
                    {
